...
/*   Updated: 2022/10/10 11:21:45 by soubella         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_state_holder	*find_state_for(t_states_holder *states, int fd)
{
	size_t	index;

	index = -1;
	while (++index < states->size)
		if (states->states[index].fd == fd)
			return (states->states + index);
	states->states[index].fd = fd;
	states->size += 1;
	return (states->states + index);
}
...
{
	static t_states_holder	states;
	t_string_builder		*builder;
	t_state_holder			*state;
	char					*result;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	state = find_state_for(&states, fd);
